#!/bin/bash
# ============================================================================
# F5 AI Guardrails Installer Entrypoint
# ============================================================================
...
write_log_configmap() {
  ...
  local cm_name="f5-ai-guardrails-installer-log-${job_name}"
  ...
  oc label configmap "$cm_name" \
    ...
    "f5-ai-guardrails-installer/expires-at=${expires_at}" 2>&1 || true
}
...
write_termination_message() {
  ...
  if [[ -n "$job_name" && "$job_name" != "unknown" ]]; then
    oc annotate job "$job_name" \
      --namespace default \
      --overwrite \
      "f5-ai-guardrails-installer/termination-message=$message" 2>/dev/null || true
  fi
}
...
case "$ACTION" in
  ...
  STATUS)
    log_status "running" "verifying" "Verifying deployment status..."
    verify_deployment
  ...
  UNINSTALL_DELETE_ALL)
    log_status "running" "uninstalling" "Removing F5 AI Guardrails and all data..."
  ...
  UNINSTALL_KEEP_DATA)
    log_status "running" "uninstalling" "Removing F5 AI Guardrails (keeping data volumes)..."
  ...
  UPGRADE)
    log_status "running" "upgrading" "Upgrading F5 AI Guardrails..."
    upgrade_quickstart
